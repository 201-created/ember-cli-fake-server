...
module("ember-cli-fake-server:stubRequest responses", function(hooks) {
  ...
  test("passthroughRequest paths are passed to Pretender passthrough", function(assert) {
  ...
  test("calling passthroughRequest when server is not started throws", function(assert) {
    FakeServer.stop();
